...
nombres_vms = [f"vm{i}_{dni}_{dificultad}" for i in range(1, 11)]


# Verificar si todas las VMs existen
if all(check_vm_existence(nombre_vm) for nombre_vm in nombres_vms):
    for nombre_vm in nombres_vms:
        ...
        if (nombre_vm == f"vm10_{dni}_{dificultad}"):
            package_vm(f"Boxes/{nombre_vm}.box", f"vm10-DNSServer_{dni}_{dificultad}", nombre_vm)
